<div class="main-sidebar sidebar-style-3 bg-success">
    <aside id="sidebar-wrapper">
        <div class="sidebar-brand">
            <a href="index.php"><a href="index.php"><img src="../img/banner/looggo-removebg-preview.png" style="width: 200px;"></a>
        </div>
        <div class="sidebar-brand sidebar-brand-sm">
            <a href="index.php">AF</a>
        </div>
        <ul class="sidebar-menu">
            <li class="menu-header">Dashboard</li>
            <li class="dropdown">
                <a href="index.php" class="nav-link"><i class="fas fa-fire"></i><span>Dashboard</span></a>
            </li>

            <li><a class="nav-link" href="notifications.php"><i class="far fa-square"></i> <span>Global Notifications</span></a></li>

            <li class="menu-header">New Buyer</li>
            <li><a class="nav-link" href="sellernew.php"><i class="far fa-square"></i> <span>Review</span></a></li>
            <li><a class="nav-link" href="masterlist.php"><i class="far fa-square"></i> <span>Farmers Masterlist</span></a></li>

            <li class="menu-header">Transaction</li>

            <li><a class="nav-link" href="pending.php"><i class="far fa-square"></i> <span>Pending</span></a></li>
            <li><a class="nav-link" href="accepted.php"><i class="far fa-square"></i> <span>Accepted</span></a></li>
            <li class="menu-header">Cash On Delivery</li>
            <li><a class="nav-link" href="shipped.php"><i class="far fa-square"></i> <span>Shipped</span></a></li>
            <li><a class="nav-link" href="Delivered.php"><i class="far fa-square"></i> <span>Delivered</span></a></li>
            <li class="menu-header">Pick-Up</li>
            <li><a class="nav-link" href="pickup.php"><i class="far fa-square"></i> <span>Pick-Up</span></a></li>
            <li class="menu-header">Products</li>
            <li><a class="nav-link" href="product.php"><i class="far fa-square"></i> <span>Products List</span></a></li>

            <li class="menu-header">Users</li>
            <li><a class="nav-link" href="customer.php"><i class="far fa-square"></i> <span>Customer</span></a></li>
            <li><a class="nav-link" href="seller.php"><i class="far fa-square"></i> <span>Seller</span></a></li>

        </ul>
    </aside>
</div>